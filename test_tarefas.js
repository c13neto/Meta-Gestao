const idAutorizacao = "90e61f39-0348-4a68-893f-ceaf166c0bf9";
const baseUrl = "https://script.google.com/macros/s/AKfycbz2fNpg8Ab5djEmFvD7TE56-G3q0U__0YpeCeINJPN-2izyxVDiWaUl5vbdQERV3tJE/exec";

fetch(`${baseUrl}?rota=tarefa&idAutorizacao=${idAutorizacao}`)
  .then(r=>r.json())
  .then(res => {
    console.log(JSON.stringify(res.devMsg, null, 2));
  });

fetch(`${baseUrl}?rota=categoria&idAutorizacao=${idAutorizacao}`)
  .then(r=>r.json())
  .then(res => {
    console.log(JSON.stringify(res.devMsg, null, 2));
  });
